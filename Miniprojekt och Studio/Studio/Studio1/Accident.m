...
t_final_cooling=45;             % sluttid kylning/ början av olycka [min]
t_final_accident = 55;          % Slutet av olyckan utan kylning

%[t_out,U_out] = ode45(@ode_function,[t_range],[initial_values],[options],constants)

% Accident with cooling
[t1, U1]=ode15s(@ode_accident_cooling, [t_start t_final_cooling], U0,[], R, Ea, DHr, A, UA, V, N0_H2O, Cp_ONCB, Cp_H2O, Cp_NH3);

%Accident no cooling
[t2, U2]=ode15s(@ode_accident, [t_final_cooling t_final_accident], U1(end,:), [], R, Ea, DHr, A, UA, V, N0_H2O, Cp_ONCB, Cp_H2O, Cp_NH3);
t = [t1; t2];
U = [U1; U2];
